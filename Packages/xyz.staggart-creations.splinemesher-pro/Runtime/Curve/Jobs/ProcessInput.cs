    ...
    public struct ProcessInput : IJob
    {
        ...
        public void Setup(CurveMeshSettings.InputMesh settings, float3 minSize, float3 offset)
        {
            ...
            if (mesh == null)
            {
                ...
                mesh = new Mesh();
                #if SM_DEV
                Debug.LogWarning("Spline Curve Mesher: No mesh found for input shape. Using dummy mesh instead.");
                #endif
            }
            
            //For runtime use, mark procedural mesh as readable
            if (Application.isPlaying && settings.shape != CurveMeshSettings.Shape.Custom)
            {
