...
namespace sc.splinemesher.pro.runtime
{
    ...
    public partial class SplineCurveMesher : SplineMesher, IDisposable
    {
        ...
        public bool InputMeshReadable()
        {
            if(Application.isPlaying == false) return true;
            
            if (settings.input.shape != CurveMeshSettings.Shape.Custom) return true;
            
            return settings.input.shape == CurveMeshSettings.Shape.Custom && settings.input.mesh && settings.input.mesh.isReadable;
        }
        ...
        public override void Rebuild(int splineIndex = -1)
        {
            ...
            if (InputMeshReadable() == false)
            {
                throw new Exception($"[Spline Curve Mesher] To use this at runtime, the mesh \"{settings.input.mesh.name}\" requires the Read/Write option enabled in its import settings. For procedurally created geometry, use \"Mesh.UploadMeshData(false)\" when creating the mesh.");
            }
        ...
        private void RebuildSpline(int splineIndex)
        {
            ...
            bool meshCreated = createMesh && meshLength >= MIN_MESH_LENGTH;
            ...
            if (createCollider)
            {
                ...
                float3 meshSize = meshCreated ? meshData.bounds.size : new float3(MIN_MESH_LENGTH, MIN_MESH_LENGTH, 1f);
        ...
        private void GenerateGeometry(NativeSpline nativeSpline, int splineIndex, SplineCurveMeshData splineMeshData, Structs.InputMeshData meshData, bool createMesh, Structs.InputMeshData colliderData, bool createCollider)
        {
            ...
            if (settings.output.segmentingMode == CurveMeshSettings.OutputMesh.Segmenting.ByLength)
            {
                ...
                segmentCount = Mathf.CeilToInt(totalMeshLength / m_maxSegmentLength);
            }
            else if (settings.output.segmentingMode == CurveMeshSettings.OutputMesh.Segmenting.EveryTile)
            {
                segmentCount = Mathf.CeilToInt(totalMeshLength / meshOccupiedLength)-1;
            }
            else if (settings.output.segmentingMode == CurveMeshSettings.OutputMesh.Segmenting.Specific)
            {
                int maxSegments = Mathf.FloorToInt(totalMeshLength / meshOccupiedLength);
                segmentCount = Mathf.Min(settings.output.segmentCount, maxSegments);
            }
            else if (settings.output.segmentingMode == CurveMeshSettings.OutputMesh.Segmenting.None)
            {
                segmentCount = 1;
            }
            ...
            //InputMeshData data is disposed later in with inputMeshProcessJob.Dispose()
        }
